    ...
    <div className="flex flex-col items-center justify-center min-h-screen bg-gray-100 p-4 text-gray-900">
      {!scanned ? (
        <div className="bg-white p-6 rounded-2xl shadow-xl text-center w-96 border border-gray-300">
          <img src="https://upload.wikimedia.org/wikipedia/commons/2/2c/PhonePe_Logo.png" alt="PhonePe Logo" width="200" className="w-24 mx-auto mb-4" />
        ...
        <div className="bg-white p-6 rounded-2xl shadow-xl text-center w-96 border border-gray-300">
          ...
          <p className="text-gray-500 text-sm">Transaction ID: {Math.floor(Math.random() * 100000000)}</p>
      ...
      {showSms && (
        <div className="fixed bottom-4 left-4 bg-gray-900 text-white p-4 rounded-lg shadow-lg w-80 animate-bounce">
          ...
          <p>₹{amount}.00 has been debited from your account via UPI. Transaction ID: {Math.floor(Math.random() * 100000000)}</p>
        </div>
      )}
    </div>
  );
